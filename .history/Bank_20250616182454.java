...
/**
 * Bank class refactored to use Unit of Work pattern with connection management.
 * Each business operation manages its own database connection.
 */
public class Bank {
    ...
    public String updateClientRank(int clientId, int newRank) {
        try (Connection conn = DatabaseManager.getConnection()) {
            if (!ClientDAO.clientExists(conn, clientId)) return "Error: Client with ID " + clientId + " not found.";
    ...
    /**
     * Removes a client from an account with enhanced destructive functionality.
     * If the client is not associated with any other accounts, the client is also deleted.
     */
    public String removeClientFromAccount(int accountNumber, int clientId) {
        try (Connection conn = DatabaseManager.getConnection()) {
            ...
            if (!isAssociatedWithOtherAccounts) {
                ...
                if (clientDeleted) {
                    result.append(" Client ").append(clientId).append(" was not associated with any other accounts and has been completely deleted from the system.");
                } else {
                    result.append(" However, failed to delete client ").append(clientId).append(" from the system.");
                }
            } else {
                result.append(" Client ").append(clientId).append(" remains in the system as they are associated with other accounts.");
            }
