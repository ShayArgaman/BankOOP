    ...
    public String getAllAccountClientAssociations() {
        try (Connection conn = DatabaseManager.getConnection()) {
            ...
            for (Account account : accounts) {
                // Always print the account header first
                sb.append(String.format("Account #%d (ID: %d, Type: %s):\n", 
                    account.getAccountNumber(), account.getId(), account.getAccountType()));

                List<Client> clients = ClientDAO.getClientsForAccount(conn, account.getId());
                
                // Check if the client list is empty and display the appropriate message
                if (clients.isEmpty()) {
